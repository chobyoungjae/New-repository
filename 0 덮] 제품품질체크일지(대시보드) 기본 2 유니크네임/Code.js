...
function onFormSubmit(e) {
  ...
  if (line && product && weight && lot) {
    ...
    try {
      ...
      if (ajId) {
        const jBlob = UrlFetchApp.fetch(`https://drive.google.com/thumbnail?sz=w400&id=${ajId}`, {
      ...
      if (akId) {
        const kBlob = UrlFetchApp.fetch(`https://drive.google.com/thumbnail?sz=w400&id=${akId}`, {
          headers: { Authorization: 'Bearer ' + ScriptApp.getOAuthToken() },
        }).getBlob();
      ...
      if (alId) {
        const kBlob = UrlFetchApp.fetch(`https://drive.google.com/thumbnail?sz=w400&id=${alId}`, {
          headers: { Authorization: 'Bearer ' + ScriptApp.getOAuthToken() },
        }).getBlob();
...
function pushToBoard(boardId, role, srcRow, url) {
  ...
  // sh.getRange(dstRow,9).setFormula(imp('O')); // << 다음 서명자                 @@ 주석처리
  // sh.getRange(dstRow,10).setFormula(imp('Q')); // << 최종 서명자                  @@ 주석처리
