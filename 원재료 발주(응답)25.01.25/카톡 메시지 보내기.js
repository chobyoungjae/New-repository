...
function onEdit(e) {
  ...
  if (col === 12 && value === "발주확인") {
    ...
    if (!logVal) {
      ...
      var result = sendKakaoTo(
        "오수진", message, friendMap, confirmUrl
      );
  ...
  if (col === 14 && value === "발주완료") {
    ...
    if (!logVal2) {
      ...
      var result2 = sendKakaoTo(
        purchaser, message2, friendMap, completeUrl
      );
...
function sendKakaoTo(name, message, friendMap, linkUrl) {
  ...
  var token = SpreadsheetApp.getActiveSpreadsheet()
    .getSheetByName("UUID").getRange("F1").getValue();
  if (!token) {
    Logger.log("❌ accessToken 누락");
    return "❌ access_token 누락";
  }
  var urlToUse = linkUrl ||
    'https://kakao-test-ebon.vercel.app/go.html?doc=' + name;
  var payload = {
