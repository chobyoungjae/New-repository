...
function onFormSubmit(e) {
  ...
  Logger.log(`▶ row=${row}, raw status(col17)=[${data().getRange(row,17).getValue()}], trimmed=[${status}]`);
  ...
  if (status === '오전') {
    ...
    const formatted = Utilities.formatDate(new Date(timestamp),                  
...
function exportPdfAndNotify(row) {
  ...
  try {
    ...
    // ③ Drive에 업로드
    const file = DriveApp.getFolderById(CFG.PDF_FOLDER)
      .createFile(blob.setName(`온습도 점검표_${sheetName}.pdf`));
