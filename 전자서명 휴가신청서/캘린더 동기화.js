...
function handleEditTrigger(e) {
  ...
  // P열(20) / 값이 '이경진' / 아직 S열(22)이 비어있을 때만
  if (range.getColumn() !== 20) return;
  if (e.value !== '이경진') return;
  const row    = range.getRow();
  const status = sheet.getRange(row, 22).getValue();
  if (status) return;
...
function updateRowInCalendar(sheet, row) {
  ...
  if (!(startDate instanceof Date)) {
    sheet.getRange(row,21).setValue('시작일 오류'); return;
  }
  if (!(endDate   instanceof Date)) {
    sheet.getRange(row,21).setValue('종료일 오류'); return;
  }

  let eventExists = false;
  let eventId     = sheet.getRange(row,22).getValue();     // S
  if (eventId) {
    try {
      if (eventId.indexOf('@') === -1) eventId += '@google.com';
      const ev = cal.getEventById(eventId);
      if (ev) {
        sheet.getRange(row,21).setValue('이미 등록됨');
        return;
      }
    } catch (err) {
      sheet.getRange(row,21).setValue('조회 오류'); return;
    }
  }
  ...
  try {
    ...
    const newEv = (startDate.getTime() === endDate.getTime())
    ...
    sheet.getRange(row,22).setValue(newEv.getId());    // S
    sheet.getRange(row,21).setValue('등록완료');         // R

  } catch (err) {
    sheet.getRange(row,21).setValue('오류: ' + err.message);
  }
}
