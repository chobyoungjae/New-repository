function doGet(e) {
  ...
  if (e.parameter.action === 'list') {
    ...
    var displayData = sheet.getDataRange().getDisplayValues(); // <-- 추가!
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      docs.push({
        ...
        teamLeaderSignUrl: displayData[i][26], // AA열 (27번째 컬럼, 0부터 시작)
        reviewerSignUrl: displayData[i][27], // AB열 (28번째 컬럼)
        ceoSignUrl: displayData[i][28], // AC열 (29번째 컬럼)
    ...
    return ContentService.createTextOutput(JSON.stringify(docs)).setMimeType(
  ...
  if (e.parameter.action === 'getDoc' && e.parameter.author) {
    ...
    if (!sheet) {
      return ContentService.createTextOutput(JSON.stringify({ error: '시트 없음' })).setMimeType(
        ContentService.MimeType.JSON
      );
    }
    var data = sheet.getDataRange().getValues();
    if (data.length < 2) {
      return ContentService.createTextOutput(JSON.stringify({ error: '데이터 없음' })).setMimeType(
        ContentService.MimeType.JSON
      );
    }
    var headers = data[0];
    ...
    return ContentService.createTextOutput(JSON.stringify(doc)).setMimeType(
      ContentService.MimeType.JSON
    );
  }
