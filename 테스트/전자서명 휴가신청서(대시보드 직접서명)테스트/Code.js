...
function onFormSubmit(e) {
  ...
  if (owner) {
    ...
    sheetUrl =
      ss.getUrl().replace(/\/edit.*$/, "") + `/edit?gid=${s.getSheetId()}`; // << 개인 시트 URL 생성
  }
...
function saveSignature(dataUrl, row, role) {
  ...
  // 이름 가져오기
  const name = data().getRange(row, 2).getDisplayValue().trim();
  // 파일명: 이름_역할.png
  ...
  Logger.log(
    `[saveSignature] setFormula DONE row: ${row}, col: ${col}, role: ${role}`
  );
  ...
  if (role === "leader") {
    data()
      ...
      .setFormula(
        `=IFERROR(VLOOKUP(B${row}, '${CFG.LOOKUP}'!B:H, 6, FALSE),"")`
      );
    ...
    if (info)
      pushToBoard(info.boardId, "reviewer", row, getPersonalSheetUrl(row));
  } else if (role === "reviewer") {
    data()
      .getRange(row, CFG.COL.CEO)
      .setFormula(
        `=IFERROR(VLOOKUP(B${row}, '${CFG.LOOKUP}'!B:H, 7, FALSE),"")`
      );
    SpreadsheetApp.flush();
...
function updateRowInCalendar(sheet, row) {
  ...
  const title = `${sheet.getRange(row, 2).getValue()} ${sheet
    .getRange(row, 6)
    .getValue()} ${sheet.getRange(row, 3).getValue()}`; // << 일정 제목 생성
