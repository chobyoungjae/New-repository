...
function getKakaoFriendsToSheet() {
  ...
  try {
    ...
    const now = new Date();
    sheet.getRange("A1").setValue("📥 친구 목록 갱신: " + Utilities.formatDate(now, Session.getScriptTimeZone(), "yyyy-MM-dd HH:mm:ss"));
    ...
    sheet.getRange("A1").setValue("❌ 오류 발생 시간: " + Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyy-MM-dd HH:mm:ss"));
...
function sendKakaoMessagesFromSheet() {
  ...
  for (let row = 2; row <= lastRow; row++) {
    ...
    try {
      const response = UrlFetchApp.fetch("https://kapi.kakao.com/v1/api/talk/friends/message/default/send", options);
